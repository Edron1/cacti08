...
function grow_graph_tree($tree_id, $start_branch, $user_id, $options) {
	...
	foreach ($hierarchy as $leaf) {
		...
		if (isset($hierarchy[$i+1])) {
			if ($hierarchy[$i+1]["title"] != "") {
				$next_leaf_type = "heading";
			} elseif (!empty($hierarchy[$i+1]["local_graph_id"])) {
				$next_leaf_type = "graph";
			} else {
				$next_leaf_type = "host";
			}
		} else {
...
$(function () {
	$("#jtree")
		.jstree({
		        <?php
/*
  The jquery code to build a collapsable tree, doesn't provide working links when jquery.js is installed,
  but jquery.cookie.js is not (Debian bug 708001 http://bugs.debian.org/708001).
*/
        if (file_exists('/usr/share/javascript/jquery-cookie')) {
	  print "\"plugins\" : [\"ui\",\"themes\",\"html_data\",\"cookies\"],\n";
	} else {
	  print "\"plugins\" : [\"ui\",\"themes\",\"html_data\"],\n";
	}
