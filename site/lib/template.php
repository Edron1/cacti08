...
function push_out_data_source_item($data_template_rrd_id) {
	...
	foreach ($struct_data_source_item as $field_name => $field_array) {
		...
		if (((empty($data_template_rrd["t_" . $field_name])) || (preg_match("/FORCE:/", $field_name))) && 
			(isset($data_template_rrd["t_" . $field_name]) && isset($data_template_rrd[$field_name]))) {
...
function push_out_data_source($data_template_data_id) {
	...
	foreach ($struct_data_source as $field_name => $field_array) {
		/* are we allowed to push out the column? */
		if (((empty($data_template_data["t_" . $field_name])) || (preg_match("/FORCE:/", $field_name))) && 
			(isset($data_template_data["t_" . $field_name]) && isset($data_template_data[$field_name]))) {
			
			// Подготовленное выражение для обновления данных
			$stmt = $pdo->prepare("UPDATE data_template_data SET $field_name = :field_value WHERE local_data_template_data_id = :id");
			$stmt->execute([
				':field_value' => $data_template_data[$field_name],
				':id' => $data_template_data["id"]
			]);
...
function change_data_template($local_data_id, $data_template_id) {
	...
	while (list($field_name, $field_array) = each($struct_data_source)) {
		if ((isset($data[$field_name])) || (isset($template_data[$field_name]))) {
			// Проверка на значение в template_data с использованием квадратных скобок
			if ((!empty($template_data["t_" . $field_name])) && ($new_save == false)) {
				$save[$field_name] = $data[$field_name];
			} else {
				// В случае отсутствия данных из $data, используем данные из $template_data
				if (isset($template_data[$field_name])) {
