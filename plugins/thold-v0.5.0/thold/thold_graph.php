	...
	if (sizeof($hosts)) {
		foreach ($hosts as $host) {
			...
			if ($host["availability_method"] != 0) {
				...
				<td>
					<?php 
					print (strlen($_REQUEST["filter"]) ? preg_replace("/(" . preg_quote($_REQUEST["filter"], '/') . ")/i", "<span style='background-color: #F8D93D;'>\\1</span>", $host["description"]) : $host["description"]);
					?>
				</td>
				...
				<td>
					<?php 
					print (strlen($_REQUEST["filter"]) ? preg_replace("/(" . preg_quote($_REQUEST["filter"], '/') . ")/i", "<span style='background-color: #F8D93D;'>\\1</span>", $host["hostname"]) : $host["hostname"]);
					?>
				</td>
